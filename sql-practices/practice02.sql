...
-- 예) 2014년 07월 10일
select date_format(max(hire_date), '%Y년 %m월 %d일') as '신입사원이 마지막으로 들어온 날'
  from employees;
...
select min((date_format(now(),'%Y') - date_format(birth_date,'%Y'))) + 1 as '제일 어린 사원의 나이',
       max((date_format(now(),'%Y') - date_format(birth_date,'%Y'))) + 1 as '제일 많은 사원의 나이'
  from employees;